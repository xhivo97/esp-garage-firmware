...
#define GPIO_OUTPUT_PIN_SEL \
	((1ULL << RELAY_A_GPIO) | (1ULL << RELAY_B_GPIO) | (1ULL << LIGHT_GPIO))
// Input GPIOs selection mask
#define GPIO_INPUT_PIN_SEL                                 \
	((1ULL << LIMIT_A_GPIO) | (1ULL << LIMIT_B_GPIO) | \
		(1ULL << BUTTON_GPIO))
...
static void event_listener()
{
	...
	for (;;) {
		...
		timer_enabled[LIM_SW_A_DEBOUNCE_TIMER] =
			get_level(LIMIT_A_GPIO);
		timer_enabled[LIM_SW_B_DEBOUNCE_TIMER] =
			get_level(LIMIT_B_GPIO);
		timer_enabled[BUTTON_DEBOUNCE_TIMER] = get_level(BUTTON_GPIO);
		timer_enabled[MOTOR_RUN_LIMIT_TIMER] =
			garage_state == OPENING_S || garage_state == CLOSING_S;
		...
		for (int i = 0; i < NUM_TIMERS; i++) {
			...
			if ((current_tick - start_tick[i] ==
				    timer_duration[i])) {
				ESP_LOGI(TAG, "%-29s -> %ums", timer_names[i],
					TICKS_TO_MILLIS(timer_duration[i]));
				timer_enabled[i] = false;
				switch (i) {
				case BUTTON_DEBOUNCE_TIMER:
				case SW_BUTTON_COOLDOWN_TIMER:
					// Update the light timer
					start_tick[LIGHT_ON_TIMER] =
						current_tick;
				...
				case LIM_SW_A_DEBOUNCE_TIMER:
					stop_garage();
					garage_state = INVERT_LIMIT_SWITCHES ?
							       CLOSED_S :
							       OPENED_S;
					break;
				...
				case GARAGE_OPEN_TIMER: {
					char *minute =
						DOOR_OPEN_NOTIFICATION_MINUTES >
								1 ?
							"minutes" :
							"minute";
					char message[128];
		...
		if (garage_prev_state != garage_state) {
			ESP_LOGI(TAG, "%-29s -> %s",
				garage_state_name[garage_prev_state],
				garage_state_name[garage_state]);
