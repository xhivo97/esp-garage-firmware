...
void update_status(char *state)
{
	...
	esp_rmaker_param_t *status =
		esp_rmaker_device_get_param_by_name(garage_device, "Status");
...
void app_main()
{
	...
	esp_rmaker_node_t *node = esp_rmaker_node_init(
		&rainmaker_cfg, "ESP RainMaker Garage Opener", "Garage Opener");
	if (!node) {
	...
	esp_rmaker_device_add_cb(
		garage_device, garage_rainmaker_callback, NULL);
	esp_rmaker_device_add_param(
		garage_device, esp_rmaker_name_param_create(
				       ESP_RMAKER_DEF_NAME_PARAM, "Garage"));

	esp_rmaker_param_t *garage_status = esp_rmaker_param_create("Status",
		NULL, esp_rmaker_str(garage_state_name[garage_state]),
		PROP_FLAG_READ);
	esp_rmaker_param_add_ui_type(garage_status, ESP_RMAKER_UI_TEXT);

	esp_rmaker_param_t *garage_param = esp_rmaker_param_create("Button",
		NULL, esp_rmaker_bool(false), PROP_FLAG_READ | PROP_FLAG_WRITE);
